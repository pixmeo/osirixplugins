...
- (void) setText: (int) i :(NSMutableDictionary*) seg
{
	...
	if( [[c presetCoronaryArray] count] == 0)
	{
		[seg setObject: @"" forKey: @"text"];
		return;
	}
	
	if( val >= [[c presetCoronaryArray] count])
		val = [[c presetCoronaryArray] count] -1;
	
	if( [[[c.presetsList selection] valueForKey: @"CoronaryDisplayLegendSegmentsNumber"] boolValue])
		text = [text stringByAppendingFormat: @"%d\r", i+1];
		
	if( [[[c.presetsList selection] valueForKey: @"CoronaryDisplayLegendSegmentsText"] boolValue])
		text = [text stringByAppendingFormat: @"%@\r", [[[c presetCoronaryArray] objectAtIndex: val] objectForKey: @"state"]];
		
	if( [[[c.presetsList selection] valueForKey: @"CoronaryDisplaySegmentsScore"] boolValue])
		text = [text stringByAppendingFormat: @"%d\r", [[[[c presetCoronaryArray] objectAtIndex: val] objectForKey: @"score"] intValue]];
...
-(NSString*) csv:(BOOL) includeHeaders
{
	...
	if( [[[c.presetsList selection] valueForKey: @"CoronaryDisplaySegment17"] boolValue])
		segmentsTotal = 17;
	...
	{
		NSDictionary *d = [[c presetCoronaryArray] objectAtIndex: [[[segments objectAtIndex: i] objectForKey: @"state"] intValue]];
...
- (void)drawRect:(NSRect)dirtyRect
{
	...
	for( int i = 0; i < segmentsTotal; i++)
	{
		...
		if( [[s objectForKey: @"state"] intValue] >= [[c presetCoronaryArray] count] && [[c presetCoronaryArray] count] > 0)
			[s setObject: [NSNumber numberWithInt: [[c presetCoronaryArray] count]-1] forKey: @"state"];
		
		NSColor *color = [NSColor whiteColor];
		
		if( [[c presetCoronaryArray] count])
			color = [NSUnarchiver unarchiveObjectWithData: [[[c presetCoronaryArray] objectAtIndex: [[s objectForKey: @"state"] intValue]] objectForKey: @"color"]];
	...
	if( [[[c.presetsList selection] valueForKey: @"CoronaryDisplayScoreSum"] boolValue])
	{
		int maxScore = -10000;
		int maxTotal = 0, total = 0;
		
		for( NSDictionary *d in [c.presetCoronary arrangedObjects])
		{
