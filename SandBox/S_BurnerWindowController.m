...
@implementation S_BurnerWindowController
...
{
    if( self = [super initWithWindowNibName:@"S_BurnViewer"])
    {
...
- (id)initWithFiles:(NSArray *)theFiles managedObjects:(NSArray *)managedObjects
{
	if( self = [super initWithWindowNibName:@"S_BurnViewer"])
	{
		...
		dbObjectsID = [managedObjects mutableCopy]; // DicomImage
		...
		NSImage *image = [im thumbnail];
		
		DicomSeries *series = [im series];
		
		NSData *imageSeries = [series thumbnail];
