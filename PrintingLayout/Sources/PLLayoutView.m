        ...
        else
        {
            ...
            if (nbPages == 1)
            {
                ...
                if ([self updateLayoutViewWidth:newWidth height:newHeight])
                {
                    ...
                    for (NSUInteger i = 0; i < nbImages; ++i)
                    {
                        ...
                        [thumb fillView:i withDCMView:*draggedView atIndex:i];
                        ++filledThumbs;
                    }
                }
            }
            else
            {
                ...
                if ([self updateLayoutViewWidth:newWidth height:newHeight])
                {
                    ...
                    {
                        PLThumbnailView *thumb = [self.subviews objectAtIndex:i];
                        if ([thumb fillView:i withDCMView:*draggedView atIndex:i])
                            ++filledThumbs;
                    }
                }
                ...
                for (NSUInteger i = 1; i < nbPages; ++i)
                {
                    ...
                    if ([pageToFill updateLayoutViewWidth:newWidth height:newHeight])
                    {
                        ...
                        for (NSUInteger j = 0; j < nbThumbs; ++j)
                        {
                            ...
                            if ([thumb fillView:j withDCMView:*draggedView atIndex:j + 24 * i])
...
- (void)clearAllThumbnailsViews
{
    ...
    {
        PLThumbnailView *thumb = [self.subviews objectAtIndex:0];
        NSRect frame = thumb.frame;
        [thumb removeFromSuperviewWithoutNeedingDisplay];
        [self addSubview:[[PLThumbnailView alloc] initWithFrame:frame]];        
//        [thumb clearView];
//        thumb.isSelected = NO;
    }
